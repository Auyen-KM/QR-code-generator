...
class App(ct.CTk):
    def __init__(self):
        ...
        self.qr_image = ct.CTkButton(
            master=self.frame,
            width=400,
            height=400,
            text="Here will be a QR code!",
            hover=False,
        ...
        self.menu_frame = ct.CTkFrame(
            master=self.frame, width=300, height=500, fg_color="transparent"
        )
        self.menu_frame.grid(row=0, column=1)

        self.title_text = ct.CTkTextbox(
        ...
        self.wellcome_text = ct.CTkTextbox(
            master=self.menu_frame,
            width=300,
            height=140,
            fg_color="transparent",
        ...
        self.text_url = ct.CTkEntry(
            master=self.menu_frame,
            width=250,
            height=35,
            placeholder_text="Write text here or paste link",
        )
        self.text_url.pack(pady=10)

        self.name_file = ct.CTkEntry(
            master=self.menu_frame,
            width=250,
            height=35,
            placeholder_text="Write name of file here, if you want",
        )
        self.name_file.pack(pady=10)

        self.button = ct.CTkButton(
            master=self.menu_frame,
            command=self.qrcode_func,
            width=250,
            height=40,
            text="Create QR code",
            font=("Roboto", 20),
    ...
    def qrcode_func(self):
        qr = QRCode(
            version=1, error_correction=constants.ERROR_CORRECT_L, box_size=10, border=2
        )
        ...
        if self.name_file.get() == "":
            ...
            q_image = ct.CTkImage(
                dark_image=Image.open(f"QR code.png"), size=(350, 350)
            )
        else:
            qrcode_img.save(f"{self.name_file.get()}.png")
            q_image = ct.CTkImage(
                dark_image=Image.open(f"{self.name_file.get()}.png"), size=(350, 350)
            )

        self.qr_image.configure(image=q_image)
        self.qr_image.configure(width=400)  # without this it's not working idk why
